...
| 30 | **Auth hardening: email confirmation + password policy** | A | `mailer_autoconfirm: true` — anyone can register **any** email, active instantly, no ownership proof. Main impact: it **undercuts the promo rate-limit (013)**, which is keyed on `user_id` — free unlimited accounts make the per-user budget a speed bump, so code *entropy* is doing the real work. Also enables account squatting and fake-account analytics pollution. The app never checks `email_confirmed_at` anywhere either. Decide before launch: enable Auth → Providers → Email → Confirm email (adds friction), or keep autoconfirm and compensate with high-entropy codes + tight `max_redemptions`. Also: client enforces 8-char passwords but that's bypassable via direct API — check the server-side minimum (Supabase default 6) and turn on **leaked-password protection** (HaveIBeenPwned), which is off by default. audit §19 |
