...
| 27 | **Content enrichment backlog** | 🔵 | Map from the 2026-08-26 content audit. **✅ Done:** food pages now surface preparation, traditional pairings (linked), and classical citations (commit bc85e5f); "Best foods for Vata/Pitta/Kapha" dosha hub pages (in progress). **Remaining, prioritized:** (a) **in-app parity** — mirror the food-page enrichment (preparation/pairings/citations) on the app's IngredientDetailPage; (b) **"foods for a Vata imbalance" reverse index** (vikriti→foods, feature #7); (c) ⭐ **viruddha āhāra (food combinations)** data layer — only 6 foods have `combosToAvoid`; authoring classical incompatible pairings (milk+fish, curd at night, honey+heat) is the biggest unique-content moat + SEO ("can I eat X with Y"), needs founder review like the diet batches; (d) **seasonal guidance (ṛtucharyā)** — season→dosha→foods layer, `bestSeason` sparse; (e) **guides/blog** long-form top-of-funnel. Pose pages already rich (leave). |
