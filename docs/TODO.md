...
| # | Task | Who | Notes |
|---|------|-----|-------|
| 11 | **Check Play account type → start 20-tester closed test** | A | **Longest pole in the entire launch.** Personal Play accounts created after Nov 2023 must run a closed test with **20 testers for 14 continuous days** before production access. Organisation accounts are exempt. If personal, start this clock *before anything else* — everything else can proceed in parallel, this can't be compressed. growth-plan §3.1 |
