...
| 29 | **`avatars` bucket is publicly listable → enumerates user IDs** | A+C | Anyone with the anon key can list the bucket; folder names are raw user UUIDs (`${user.id}/avatar.ext`). Today = 1 folder (founder), so nil impact — **at launch every user ID becomes enumerable**, plus a live user count. Information disclosure, not a breach (RLS still needs a JWT), so P2 — but fix before launch. Bucket is public, so objects still serve via `/object/public/` **without** a SELECT policy; it's *listing* that must go. Run `select policyname, cmd, roles, qual from pg_policies where schemaname='storage' and tablename='objects';` and send me the output — I'll write the migration. Not done blind: guessing storage policy names risks breaking avatar upload/display. audit §16 |
