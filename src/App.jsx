...
const YogaNidraPage = lazy(() => import('./pages/YogaNidraPage'))
...
function DeepLinkHandler() {
  ...
  useEffect(() => {
    ...
    const processUrl = (url) => {
      ...
      // ── PKCE flow (current) ──
      // Supabase redirects directly to `com.sanctuary.app://auth-callback?code=…`
      // and we exchange the code for a session here. The PKCE
      // code_verifier was generated by signInWithOAuth() and stored in
      // localStorage — Supabase reads it back during the exchange.
      ...
      if (code) {
        ...
        supabase.auth.exchangeCodeForSession(code).then(({ error }) => {
          ...
          // On success, onAuthStateChange fires SIGNED_IN and the
          // AppRoutes guard navigates the user to /home.
        })
        return
      }

      // ── Implicit flow (legacy, kept for backward compat) ──
      // If any in-flight OAuth flows started under the old implicit
      // flow + GitHub-Pages middleman, they still arrive here with
      // tokens as query params. Honor them once, then deprecate.
      ...
      if (accessToken && refreshToken) {
        ...
        supabase.auth.setSession({
          ...
          if (error) {
            console.error('OAuth session (legacy implicit) error:', error.message)
            setTransitioning(false)
          }
        })
      }
    ...
    <>
      ...
      <PageTransition>
        <Routes>
          ...
          <Route path="/meal/:id" element={<PrivateRoute><MealDetailPage /></PrivateRoute>} />
          <Route path="/nidra" element={<PrivateRoute><YogaNidraPage /></PrivateRoute>} />
          ...
          <Route path="/protocol/:vikriti" element={<PrivateRoute><ProtocolPage /></PrivateRoute>} />
